...
import { t, formatEquipmentName } from '../../i18n.js';
...
export class HeroDetailsView extends View {
    ...
    render() {
        ...
        if (!hero) return `<div>${t('v2_hero_not_found')}</div>`;
    ...
    renderTabs() {
        return `
            <div class="tab-nav">
                <button class="tab-btn ${this.activeTab === 'stats' ? 'active' : ''}" data-tab="stats">${t('v2_stats')}</button>
                <button class="tab-btn ${this.activeTab === 'skills' ? 'active' : ''}" data-tab="skills">${t('v2_skills')}</button>
                <button class="tab-btn ${this.activeTab === 'equipment' ? 'active' : ''}" data-tab="equipment">${t('v2_gear')}</button>
            </div>
        `;
    }
    ...
    renderStats(hero) {
        const stats = [
            { id: 'baseMaxHp', label: t('stat_hp'), val: hero.maxHp, icon: '❤️' },
            { id: 'baseMaxMp', label: t('stat_mp'), val: hero.maxMp, icon: '💧' },
            { id: 'baseStrength', label: t('stat_attack'), val: hero.strength, icon: '⚔️' },
            { id: 'baseSpeed', label: t('stat_speed'), val: hero.speed, icon: '⚡' },
            { id: 'baseDefense', label: t('stat_defense'), val: hero.defense, icon: '🛡️' },
            { id: 'baseMagicPower', label: t('stat_magic'), val: hero.magicPower, icon: '✨' }
        ];

        return `
            <div class="card">
                <div class="card-title"><span>📊</span> ${t('v2_stats')} <span style="margin-left: auto; font-size: 0.8rem; color: var(--text-dim);">${t('v2_points')}: ${hero.statPoints}</span></div>
    ...
    renderSkills(hero) {
        ...
        return `
            <div class="card">
                <div class="card-title"><span>🛡️</span> ${t('v2_skills')} <span style="margin-left: auto; font-size: 0.8rem; color: var(--text-dim);">${t('v2_points')}: ${hero.skillPoints}</span></div>
                        ...
                        return `
                            <div class="skill-item ${isUnlocked ? 'unlocked' : ''}">
                                <div class="skill-header">
                                    ...
                                    <span class="skill-level">${isUnlocked ? t('level_label') + ' ' + level : t('v2_locked')}</span>
                                </div>
                                ...
                                <div style="display: flex; justify-content: flex-end; margin-top: 10px;">
                                    <button class="btn-v2" style="font-size: 0.7rem; padding: 5px 10px;"
                                        ...
                                        ${isUnlocked ? t('v2_upgrade') : t('v2_unlock')} (${upgradeCost} ${t('v2_sp')})
                                    </button>
                                </div>
                            </div>
                        `;
    ...
    renderEquipment(hero) {
        const slots = [
            { id: 'head', label: t('slot_head') },
            { id: 'body', label: t('slot_body') },
            { id: 'legs', label: t('slot_legs') },
            { id: 'rightHand', label: t('slot_rightHand') },
            { id: 'leftHand', label: t('slot_leftHand') },
            { id: 'accessory', label: t('slot_accessory') }
        ];

        return `
            <div class="card">
                <div class="card-title"><span>🎒</span> ${t('v2_gear')}</div>
                    ...
                    ${slots.map(slot => {
                        ...
                        return `
                            <div class="equip-slot ${item ? 'occupied' : ''}">
                                ...
                                <div class="item-name">${item ? formatEquipmentName(item) : '---'}</div>
